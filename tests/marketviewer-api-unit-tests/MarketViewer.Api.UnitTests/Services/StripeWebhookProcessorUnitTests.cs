...
public class StripeWebhookProcessorUnitTests
{
    ...
    [Fact]
    public async Task InvoicePaid_UpgradeProrationForCurrentSubscriber_IsMoneyOnly()
    {
        // The upgrade invoice names the higher tier the subscription.updated event already
        // granted (or is about to); with the user still on Pro it must not double-grant.
    ...
    [Fact]
    public async Task InvoicePaid_DowngradeBilledAsSubscriptionUpdate_AppliesLowerTier()
    {
        // A scheduled period-end downgrade whose phase switch Stripe bills as
        // subscription_update: the lower tier has no other path onto the user record.
