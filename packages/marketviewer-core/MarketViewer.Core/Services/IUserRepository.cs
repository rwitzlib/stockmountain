...
public interface IUserRepository
{
    ...
    /// the tier grant (monthly credits do not roll over). Marks the subscription active.
    /// </summary>
    Task<bool> ApplySubscriptionGrant(string id, UserRole role, float monthlyGrant);

    /// <summary>
    /// Applies an immediate mid-cycle upgrade: sets the new role and MaxCredits, and adds the
    /// grant difference on top of the remaining monthly balance.
    /// </summary>
    Task<bool> ApplyUpgradeGrant(string id, UserRole role, float monthlyGrant, float creditsDelta);
